...
public class NIOShuffleOutputClient {
    ...
    public NIOShuffleOutputClient(String host, int port) {
        this.host = host;
        this.port = port;
    ...
    public void main(String[] args) {
        int threadNumber = 10;
        ThreadFactoryBuilder daemonThreadFactoryBuilder = new ThreadFactoryBuilder().setDaemon(true)  ;
        ThreadFactory threadFactory = daemonThreadFactoryBuilder.setNameFormat("thread-%d").build();
        ...
        for (int i=0; i<threadNumber; i++) {
            ...
            executorService.submit(new Runnable() {
                @Override
                public void run() {
                    NIOShuffleOutputClient client = new NIOShuffleOutputClient("localhost", 9026);
                    try {

                        client.sendFile("/home/cxli/Documents/DevelopingApplicationsWithYarn.pdf", "/tmp/test/thread" + index);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
